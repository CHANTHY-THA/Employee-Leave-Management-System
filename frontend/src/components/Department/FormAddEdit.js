...
function AddEditForm(props) {
    ...
    const submitFormAdd = (e) => {
        e.preventDefault();
        axios.post(process.env.REACT_APP_URL + '/department', department, headers, { validateStatus: () => true }).then((res) => {
            if (res.data.id > 0) {
    ...
    const submitFormEdit = (e) => {
        e.preventDefault();
        axios.put(process.env.REACT_APP_URL + '/department', department, headers, { validateStatus: () => true })
            .then((res) => {
                if (res.data.id > 0) {
